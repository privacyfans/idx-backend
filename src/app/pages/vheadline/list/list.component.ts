...
export class ListComponent implements OnInit,OnDestroy  {
  ...
  async getAllList() {
    ...
    this._vheadlineService.getAllList().subscribe(data => {
      this.vheadline = data;
      this.dtTrigger.next();
  ...
  deleteConfirm(obj) {
    Swal({
      ...
      if (result.value) {
        this._vheadlineService.delete(obj.Id)
          .subscribe(res => {
            this.vheadlines.splice(this.vheadlines.indexOf(obj), 1);
            this.dtTrigger.unsubscribe();
            this.getAllList();
            Swal(
              'Deleted!',
